...
#include <iostream> 
#include <exception>
/* Replaced mutex with TMutex */
/* since cint is not c++11 but mutex is*/
#include <TMutex.h>
#include "Json.h"
...
class HTTP {
	...
	HTTP(const HTTP &ref){std::cerr<<"HTTP: no copy ctor\n";throw -1;}
	HTTP& operator=(const HTTP&){std::cerr<<"HTTP: no assignment\n";throw -1;}
        ~HTTP();

        /// DEPRECATED
        /// Generic request that parses the result in JsonObject.
        bool request(const char* method, const char* endUrl, const char* data, JsonObject* root, const char* content_type = _APPLICATION_JSON);

        /// Generic request that parses the result in JsonObject.
        unsigned int request(const char* method, const char* endUrl, const char* data, JsonObject* root, Result& result, const char* content_type = _APPLICATION_JSON);
        ...
        inline unsigned int get(const char* endUrl, const char* data, JsonObject* root){
            Result result;
            return request("GET", endUrl, data, root, result);
        }

        /// Generic head request to node.
        inline unsigned int head(const char* endUrl, const char* data, JsonObject* root){
            Result result;
            return request("HEAD", endUrl, data, root, result);
        }

        /// Generic put request to node.
        inline unsigned int put(const char* endUrl, const char* data, JsonObject* root){
            Result result;
            return request("PUT", endUrl, data, root, result);
        }

        /// Generic post request to node.
        inline unsigned int post(const char* endUrl, const char* data, JsonObject* root){
            Result result;
            return request("POST", endUrl, data, root, result);
        }

        /// Generic delete request to node.
        inline unsigned int remove(const char* endUrl, const char* data, JsonObject* root){
            Result result;
            return request("DELETE", endUrl, data, root, result);
        }

        /// Generic post request to node.
        inline unsigned int rawpost(const char* endUrl, const char* data, JsonObject* root){
            Result result;
